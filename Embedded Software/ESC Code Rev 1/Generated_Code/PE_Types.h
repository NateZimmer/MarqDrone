...
#define EnterCritical() \
 do {\
  ...
  /*lint -save  -e586 -e950 Disable MISRA rule (2.1,1.1) checking. */\
     asm (PUSH {R0,R1});asm(MRS R0, PRIMASK);asm(CPSID i);asm(LDA R1, SR_reg);asm(STRB R0, [R1]);asm(POP {R0,R1});\
...
#define ExitCritical() \
 do {\
   if (--SR_lock == 0u) { \
  /*lint -save  -e586 -e950 Disable MISRA rule (2.1,1.1) checking. */\
     asm (PUSH {R0,R1});asm(LDA R1, SR_reg);asm(LDRB R0, [R1]);asm(MSR PRIMASK,R0;);asm(POP {R0,R1});\
  /*lint -restore Enable MISRA rule (2.1,1.1) checking. */\
   }\
 } while(0)
