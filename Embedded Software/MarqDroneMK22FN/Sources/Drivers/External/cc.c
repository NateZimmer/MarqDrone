...
void spi_setup(void)
{
	//spi_init();//SPI is already INITED
//  CSn_PxOUT |= CSn_PIN;
...
void cc_write_reg(uint8_t addr, uint8_t value)
{
	...
	// Send address
	spi_send_recv_CC(addr);

	// Send value
	spi_send_recv_CC(value);
...
void cc_write_burst_reg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  // Send address
  spi_send_recv_CC(addr | TI_CCxxx0_WRITE_BURST);

  // Send burst data
  for (i = 0; i < count; i++) {
	  // Send the data
	  spi_send_recv_CC(buffer[i]);
  }
...
uint8_t cc_read_reg(uint8_t addr)
{
  ...
  // Write the address
  spi_send_recv_CC(addr | TI_CCxxx0_READ_SINGLE);

  // Dummy write in order to receive data
  x = spi_send_recv_CC((uint8_t)0);
...
void cc_read_burst_reg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  // Send address
  spi_send_recv_CC(addr | TI_CCxxx0_READ_BURST);

  // Receive burst data
  for (i = 0; i < count; i++) {
	  //Dummy write in order to receive incoming data
	  buffer[i] = spi_send_recv_CC((uint8_t)0);
  }
...
//  volatile Chip_Status = 0;
  ...
  status = spi_send_recv_CC((uint8_t)0);
...
uint8_t cc_read_status_byte()
{
  ...
  status = spi_send_recv_CC(TI_CCxxx0_RSSI | TI_CCxxx0_READ_BURST);

  // Dummy write to read the data
  // TODO Check this with NATE
  dummy = spi_send_recv_CC((uint8_t)0);
  if (dummy) {
	  dummy++;
  }
...
//	CSn_PxOUT |= CSn_PIN;
	...
	// Send stobe
	spi_send_recv_CC(TI_CCxxx0_SRES);
	WAIT1_Waitms(10);
